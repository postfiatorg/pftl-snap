...
export type TranslateFn = ConfigTypes extends {
    TranslateFn: infer T;
} ? T : (w: string, opts?: Record<string, string>) => string;
export interface CoreComponentConfig<P> {
    defaultProps: Partial<P>;
}
export interface ComponentConfig<P> extends CoreComponentConfig<P> {
}
export type ComponentsConfig = CoreConfigTypes extends {
    ComponentsConfig: infer T;
} ? T : Record<string, ComponentConfig<any> & unknown>;
