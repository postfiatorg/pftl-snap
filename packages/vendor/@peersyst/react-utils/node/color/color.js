...
exports.deemphasize = deemphasize;
...
function clamp(value) {
  ...
  if (process.env.NODE_ENV !== "production") {
    if (value < min || value > max) {
      console.error("The value provided ".concat(value, " is out of range [").concat(min, ", ").concat(max, "]."));
    }
  }
...
function decomposeColor(color) {
  ...
  if (type === "color") {
    ...
    if (!colorSpace || ["srgb", "display-p3", "a98-rgb", "prophoto-rgb", "rec-2020"].indexOf(colorSpace) === -1) {
      throw new Error("MUI: unsupported `%s` color space." + colorSpace + "\n" + "The following color spaces are supported: srgb, display-p3, a98-rgb, prophoto-rgb, rec-2020.");
    }
  } else {
    values = values.split(",");
  }
...
function recomposeColor(color) {
  ...
  if (type.indexOf("rgb") !== -1) {
    ...
    values = colorValues.map(function (n, i) {
