...
export default function TokenSelectItem({ token }: TokenSelectItemProps) {
  const displayCurrency = getDisplayCurrencyCode(token.currency);
  return (
    <Row alignItems="center" gap="0.75rem" flex={1}>
      ...
      <Col gap="0.5rem">
        ...
        <Balance balance={token.balance.formatAmount()} currency={displayCurrency} variant="body1" fontWeight={400} light />
      </Col>
    </Row>
  );
}
